...
class YouTube:
    ...
    @kekik_cache(ttl=CACHE_TIME)
    async def lives(self, id: str) -> list[dict]:
        """Attempt to list current lives by scraping the live filter tab.
        This is best-effort when yt-dlp mode is disabled.
        """
        urls = [
            f"https://www.youtube.com/channel/{id}/videos?view=2&live_view=501",
            f"https://www.youtube.com/channel/{id}/streams",
        ]

        for url in urls:
            r = await self.oturum.get(url, follow_redirects=True)
            if r.status_code != 200:
                continue
            html = r.text
            # Find LIVE grid items by looking for title and videoId near each other
            lives: list[dict] = []
            for m in _re.finditer(r'"gridVideoRenderer":\{.*?"videoId":"([^"]+)".*?"title":\{"runs":\[\{"text":"([^"]+)"\}\]', html, _re.DOTALL):
